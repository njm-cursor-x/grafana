...
import { logStructured, config, isMigrationHandler, migrateRequest, toDataQueryError, isExpressionReference } from '@grafana/runtime';
...
export function runRequest(
  ...
  const dataObservable = callQueryMethodWithMigration(datasource, request, queryFunction).pipe(
    ...
    // handle errors
    catchError((err) => {
      logStructured('features.query', 'error', 'runRequest.catchError', err);
      return of({
        ...state.panelData,
