  ...
  protected loadScriptedDashboard(file: string): Promise<DashboardDTO> {
    ...
    return getBackendSrv()
      ...
      .then(
        ...
        (err) => {
          reportDashboardLoadFailure(err, 'Script dashboard error');
      ...
      promise = getDashboardAPI('v1').then(async (api) => {
        try {
          ...
          if (isFetchError(e) && !(e instanceof DashboardVersionError)) {
            reportDashboardLoadFailure(e, 'Failed to load dashboard');
            e.isHandled = true;
            if (e.status === 404) {
...
function reportDashboardLoadFailure(error: unknown, message: string) {
  logError(error instanceof Error ? error : new Error(message), {
    source: 'dashboard.load',
    status: isFetchError(error) ? String(error.status) : undefined,
  });
}
