...
import { logStructured, config, getBackendSrv, isFetchError, locationService } from '@grafana/runtime';
  ...
  protected loadScriptedDashboard(file: string): Promise<DashboardDTO> {
    ...
    return getBackendSrv()
      ...
      .then(
        ...
        (err) => {
          logStructured('features.dashboard', 'error', 'Script dashboard error ' + err);
      ...
      promise = getDashboardAPI('v1').then(async (api) => {
        try {
          ...
          if (isFetchError(e) && !(e instanceof DashboardVersionError)) {
            logStructured('features.dashboard', 'error', 'Failed to load dashboard', e);
            e.isHandled = true;
            if (e.status === 404) {
