  ...
  for (const v of vars) {
    ...
    switch (v.type) {
      ...
      default:
        ...
        logStructured('features.dashboard', 'error', `Variable transformation not implemented: ${v.type}`);
    }
  }
  return variables;
}
...
function getVariablesV1(vars: DashboardV2Spec['variables']): VariableModel[] {
  ...
  for (const v of vars) {
    ...
    switch (v.kind) {
      ...
      default:
        // do not throw error, just log it
        logStructured('features.dashboard', 'error', `Variable transformation not implemented: ${v}`);
    }
  }
  return variables;
}
...
function transformSpecialValueMatchToV1(match: SpecialValueMatch): SpecialValueMatchV1 | undefined {
  switch (match) {
    ...
    default:
      logStructured('features.dashboard', 'warn', `Skipping special value mapping with unknown match type: "${match}"`);
      return undefined;
  }
}
