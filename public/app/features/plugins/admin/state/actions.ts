...
import { logStructured, config, getBackendSrv, isFetchError } from '@grafana/runtime';
  ...
  try {
    ...
    const remote$ = from(getRemotePlugins()).pipe(
      catchError((err) => {
        ...
        logStructured('features.plugins', 'error', err);
        return of([]);
      })
    );
    ...
    forkJoin({
      ...
      .subscribe(
        ...
        (error) => {
          logStructured('features.plugins', 'info', error);
...
>(`${STATE_PREFIX}/install`, async ({ id, version, installType = PluginStatus.INSTALL }, thunkApi) => {
  ...
  try {
    ...
    logStructured('features.plugins', 'error', e);
    if (isFetchError(e)) {
