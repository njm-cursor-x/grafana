...
import { logStructured, TimeRangeUpdatedEvent } from '@grafana/runtime';
...
export class DashboardModelCompatibilityWrapper {
  ...
  public removePanel(panel: PanelModelCompatibilityWrapper) {
    ...
    if (!vizPanel) {
      logStructured('features.dashboard-scene', 'error', 'Trying to remove a panel that was not found in scene', panel);
      return;
    }
