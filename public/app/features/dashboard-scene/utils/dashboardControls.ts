...
async function loadControlsFromRef(ref: DataSourceRef, subscriber: Subscriber<DefaultControlEvent>) {
  ...
  try {
    ...
    logStructured('features.dashboard-scene', 'warn', 'Failed to load datasource', ref, e);
    return;
  }
...
async function emitDefaultVariables(ds: DataSourceApi, subscriber: Subscriber<DefaultControlEvent>) {
  ...
  try {
    ...
    logStructured('features.dashboard-scene', 'warn', 'Failed to load default variables from datasource', ds.type, e);
  }
}
...
async function emitDefaultLinks(ds: DataSourceApi, subscriber: Subscriber<DefaultControlEvent>) {
  ...
  try {
    ...
    logStructured('features.dashboard-scene', 'warn', 'Failed to load default links from datasource', ds.type, e);
  }
}
