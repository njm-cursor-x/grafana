...
import { getGrafanaLiveSrv, locationService, logWarning } from '@grafana/runtime';
        ...
        switch (action) {
          ...
          case DashboardEventAction.Saved: {
            ...
            if (dash?.uid !== event.message.uid) {
              logWarning('Received dashboard event for different dashboard', {
                eventDashboardUid: event.message.uid,
