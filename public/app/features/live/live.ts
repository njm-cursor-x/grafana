...
import { logStructured, type BackendSrv, type GrafanaLiveSrv } from '@grafana/runtime';
  ...
  getDataStream: GrafanaLiveSrv['getDataStream'] = (options) => {
    ...
    const updateBuffer = (next: StreamingDataQueryResponse): void => {
      ...
      if (!buffer && !isStreamingResponseData(data, StreamingResponseDataType.FullFrame)) {
        logStructured('features.live', 'warn', `expected first packet to contain a full frame, received ${data?.type}`);
        return;
      }
