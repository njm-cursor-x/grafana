...
export class ScopesApiClient {
  ...
  private extractDataOrHandleError<T>(result: { data?: T; error?: unknown }, context: string): T | undefined {
    if ('data' in result && result.data) {
      ...
      if (this.isStatusError(result.data)) {
        ...
        logStructured('features.scopes', 'error', `Failed to fetch %s:`, context, errorMessage);
        return undefined;
      }
      return result.data;
    }
  ...
  async fetchScope(name: string): Promise<Scope | undefined> {
    ...
    try {
      ...
      logStructured('features.scopes', 'error', 'Failed to fetch scope:', name, errorMessage);
      return undefined;
    } finally {
  ...
  async fetchMultipleScopes(scopesIds: string[]): Promise<Scope[]> {
    ...
    try {
      ...
      const errorMessage = getMessageFromError(err);
      logStructured('features.scopes', 'error', 'Failed to fetch multiple scopes:', scopesIds, errorMessage);
      return [];
    }
  }
  ...
  async fetchMultipleScopeNodes(names: string[]): Promise<ScopeNode[]> {
    ...
    try {
      ...
      if ('error' in result) {
        const errorMessage = getMessageFromError(result.error);
        logStructured('features.scopes', 'error', 'Failed to fetch multiple scope nodes:', names, errorMessage);
      }
  ...
  async fetchNodes(options: { parent?: string; query?: string; limit?: number }): Promise<ScopeNode[]> {
    ...
    try {
      ...
      if ('error' in result) {
        ...
        logStructured('features.scopes', 'error', 'Failed to fetch scope nodes:', context, errorMessage);
      }
  ...
  public fetchDashboards = async (scopeNames: string[]): Promise<ScopeDashboardBinding[]> => {
    ...
    try {
      ...
      if ('error' in result) {
        const errorMessage = getMessageFromError(result.error);
        logStructured('features.scopes', 'error', 'Failed to fetch dashboards for scopes:', scopeNames, errorMessage);
      }
  ...
  public fetchDefaultScope = async (): Promise<string | undefined> => {
    ...
    try {
      ...
      const errorMessage = getMessageFromError(err);
      logStructured('features.scopes', 'error', 'Failed to fetch default scope:', errorMessage);
      return undefined;
    } finally {
      subscription.unsubscribe();
    }
  ...
  public fetchScopeNode = async (scopeNodeId: string): Promise<ScopeNode | undefined> => {
    ...
    try {
      ...
      const errorMessage = getMessageFromError(err);
      logStructured('features.scopes', 'error', 'Failed to fetch scope node:', scopeNodeId, errorMessage);
      return undefined;
    } finally {
      // Unsubscribe for extra safety, even though with subscribe: false and awaiting,
      // the request completes before return, so this is mostly a no-op
