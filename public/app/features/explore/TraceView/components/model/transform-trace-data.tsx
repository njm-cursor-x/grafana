...
export default function transformTraceData(data: TraceResponse | undefined): Trace | null {
  ...
  for (let i = 0; i < max; i++) {
    ...
    if (idCount != null) {
      ...
      logStructured('features.explore', 'warn', `Dupe spanID, ${idCount + 1} x ${spanID}`, span, spanMap.get(spanID));
      if (_isEqual(span, spanMap.get(spanID))) {
        // eslint-disable-next-line no-console
        logStructured('features.explore', 'warn', '\t two spans with same ID have `isEqual(...) === true`');
      }
