...
describe('JourneyRegistryImpl', () => {
  ...
  it('should warn for registry entries with no start trigger registered', () => {
    const logger = mockLogger('core.journeys');
    ...
    expect(logger.logWarning).toHaveBeenCalledWith('Registry entry has no triggers registered', {
      journeyType: 'another_journey',
    });
  });
