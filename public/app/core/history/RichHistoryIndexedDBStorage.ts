...
import { reportInteraction, TracedError } from '@grafana/runtime';
...
export default class RichHistoryIndexedDBStorage implements RichHistoryStorage, IndexedDBMigrationAccess {
  ...
  private ensureMigrated(): Promise<void> {
    if (!this.migrationPromise) {
      this.migrationPromise = migrateToIndexedDB(this).catch((error) => {
        ...
        getLogger('features.query-history.indexed-db').logError(
          new TracedError('Query history migration failed', error),
          { operation: 'migrateToIndexedDB' }
        );
  ...
  async getRichHistory(filters: RichHistorySearchBackendFilters): Promise<RichHistoryResults> {
    ...
    void this.maybeRunRetentionCleanup(db).catch((error) => {
      getLogger('features.query-history.indexed-db').logError(
        new TracedError('Query history retention cleanup failed', error),
