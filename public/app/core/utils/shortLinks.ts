...
import { logStructured, getBackendSrv, config, locationService } from '@grafana/runtime';
...
export const createShortLink = memoizeOne(async (path: string): Promise<string> => {
  try {
    ...
    logStructured('core.utils', 'error', 'Error when creating shortened link: ', err);
...
export const createAndCopyShortLink = async (path: string) => {
  try {
    ...
    logStructured('core.utils', 'error', 'Error in createAndCopyShortLink:', error);
  }
};
