...
import { getBackendSrv, config, locationService, TracedError } from '@grafana/runtime';
...
export const createShortLink = memoizeOne(async (path: string): Promise<string> => {
  try {
    ...
    getLogger('core.short-links').logError(new TracedError('Failed to create shortened link', err), {
...
export const createAndCopyShortLink = async (path: string) => {
  try {
    ...
    getLogger('core.short-links').logError(new TracedError('Failed to copy shortened link', error), {
      operation: 'createAndCopyShortLink',
    });
  }
};
