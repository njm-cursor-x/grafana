...
import { logStructured, config, getGrafanaLiveSrv } from '@grafana/runtime';
...
export class LivePanel extends PureComponent<Props, State> {
  ...
  streamObserver: PartialObserver<LiveChannelEvent> = {
    next: (event: LiveChannelEvent) => {
      if (isLiveChannelStatusEvent(event)) {
        ...
        logStructured('plugins.panel', 'info', 'ignore', event);
      }
    },
  };
  ...
  async loadChannel() {
    ...
    if (!isValidLiveChannelAddress(addr)) {
      logStructured('plugins.panel', 'info', 'INVALID', addr);
      this.unsubscribe();
      this.setState({
        addr: undefined,
      });
    ...
    if (isEqual(addr, this.state.addr)) {
      logStructured('plugins.panel', 'info', 'Same channel', this.state.addr);
      return;
    }
    ...
    logStructured('plugins.panel', 'info', 'LOAD', addr);
