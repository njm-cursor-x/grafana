...
`no-console` is an ESLint **error** for `public/app/**` (tests, webpack configs, the Echo browser-console backend, `createDebugLog`, and dashboard `performanceUtils` are allowlisted). The shared `@grafana/eslint-config` allows `console.log` / `warn` / `error` / `info`; the app override replaces that list (you may see a `grafanaStructuredLoggingSentinel` placeholder in the ESLint message — do not use it).
