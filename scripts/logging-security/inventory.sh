...
LOG_PAT='console\.(log|debug|info|warn|error)|fmt\.Print'
...
echo "## 1) console.* / fmt.Print under pkg and public/app" | tee -a "$REPORT"
echo | tee -a "$REPORT"
...
echo "## 3) files that contain BOTH a log site and a secret-like identifier" | tee -a "$REPORT"
echo | tee -a "$REPORT"
