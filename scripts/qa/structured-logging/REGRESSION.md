...
**Epic integration:** this file and `scripts/qa/structured-logging/*` plus the Playwright smoke spec were copied as QA *evidence* into the epic. PR #35 is **not** the product merge source — that QA branch already embedded Backend+Frontend as a snapshot. Product code on the epic comes from lanes #30–#34 merged in order.

The table below is the QA lane’s recorded run (Backend `#31` + Frontend `#34` on that branch). Env-blocked items (`go test`, e2e, live Grafana) were not re-run as product fails.
