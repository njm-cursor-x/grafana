...
func (cl *ConcreteLogger) Log(ctx ...any) error {
	// Emit-time redaction so Authorization / cookies / tokens never reach sinks
	// even if a caller logs the raw header. Construction-time RedactFields in
	// newConcreteLogger/with still scrubs logger context before it is stored.
