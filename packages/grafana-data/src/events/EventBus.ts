...
export class EventBusSrv implements EventBus, LegacyEmitter {
  ...
  emit<T>(event: AppEvent<T> | string, payload?: T): void {
    // writeStructuredLog('grafana-data', 'info', `Deprecated emitter function used (emit), use $emit`);
  ...
  on<T>(event: AppEvent<T> | string, handler: LegacyEventHandler<T>) {
    // writeStructuredLog('grafana-data', 'info', `Deprecated emitter function used (on), use $on`);
